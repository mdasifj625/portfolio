          ...
          <div className="space-y-4 md:text-right flex flex-col md:items-end">
            ...
            <div className="flex items-center gap-4 text-muted-foreground">
              {socialLinks.map((link, i) => (
                <a key={i} href={link.url} target="_blank" rel="noreferrer" className="hover:text-primary transition-colors hover:-translate-y-1 duration-300">
                  <span className="sr-only">{link.platform}</span>
                  {getIcon(link.icon)}
                </a>
              ))}
              <a href="mailto:hello@example.com" className="hover:text-primary transition-colors hover:-translate-y-1 duration-300">
