...
- **Avatar Photo**: The "Let Me Introduce Myself" section relies on a user photo. The `avatarUrl` field in `portfolio.json` specifies its path (usually `/avatar.png`). The user must place their photo in the `/public` directory.
